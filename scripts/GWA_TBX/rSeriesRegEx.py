...
def convertDatetoDoy(dateString, lengthYearString, lengthMonthString, lengthDayString):
    ...
    try:
        day = int(
            dateString[
                lengthYearString
                + lengthMonthString : lengthYearString
                + lengthMonthString
                + lengthDayString
            ]
        )
    ...
    # Otherwise calculate doy
        ...
        doy = datetime.datetime(year=year, month=month, day=day) - datetime.datetime(
...
def getFiles(
    dataDir, filenameFormat, outputFileFormat, groupFiles, outputFiles, groupBy
):
    ...
    if not match:
        ProcessingLog.addToLog(
            ProcessingLog.LOG_INFO, "No match for date string in filename format!"
        )
        raise GeoAlgorithmExecutionException(
            "No match for date string in filename format!"
        )
    ...
    if groupBy == 7:
        ...
        dateRegex = (
            "(?<=^.{" + str(startDateString) + "})\d{" + str(lengthDateString) + "}"
        )
    ...
    for filename in matchingFiles:
        ...
        if groupBy == 6:
            date = convertDatetoDoy(
                match.group(), lengthYearString, lengthMonthString, lengthDayString
            )
...
if len(groupFiles) == 0 or len(outputFiles) == 0:
    ProcessingLog.addToLog(
        ...
        "No matching files found! r.series will not be executed.",
    )
    raise GeoAlgorithmExecutionException(
        "No matching files found! r.series will not be executed."
    )
    ...
    for date in sorted(groupFiles.iterkeys()):
        ...
        if processing.runalg("grass7:r.series", params):
            ...
            raise GeoAlgorithmExecutionException(
                'Unable to execute script "GRASS r.series for whole directory". Check Processing log for details.'
            )
