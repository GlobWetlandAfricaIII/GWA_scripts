...
if match:
    ...
    raise GeoAlgorithmExecutionException(
        "The provided statistics file for reference period and "
            ...
            if not processing_region:
                ...
                if match:
                    ...
                    try:
                        ...
                        raise GeoAlgorithmExecutionException(
                            "A landcover class is not present "
...
for region_stats in values_as_km2:
    ...
    for lc in region_stats.keys():
        ...
        try:
            change = (
                (float(reference_stats[regionId][lc]) - float(region_stats[lc]))
                / float(reference_stats[regionId][lc])
            ) * 100.0
...
json_data = {
    "metadata": [{"reference_year": reference_period, "field_only_in_table": skip_lc}]
}
...
for regionId in regionIds:
    filename = (
        os.path.splitext(csv_file)[0] + "_" + regionId + os.path.splitext(csv_file)[1]
    )
    df = pd.DataFrame(columns=periods, index=lc_labels)
    for region in values_as_km2:
        if region["regionId"] == regionId:
            df[region["yyyy"]] = [
                region[lc] if lc in region.keys() else 0.0 for lc in lc_labels
            ]
...
for regionId in regionIds:
    ...
    df.columns = [
        "Percentage area change from " + reference_period + " (" + period + ")"
