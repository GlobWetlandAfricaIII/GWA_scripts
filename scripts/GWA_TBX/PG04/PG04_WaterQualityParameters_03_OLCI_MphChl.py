...
@alg.input(
    type=alg.BOOL, name="MPHexportMph", label="Export MPH", default=False, advanced=True
)
@alg.output(type=alg.FOLDER, name="Output_folder", label="Output directory")
def pg04waterqualityparameters03olcimphchl(
    ...
    MPHvalidPixelExpression = instance.parameterAsString(
        parameters, "MPHvalidPixelExpression", context
    )
    MPHcyanoMaxValue = instance.parameterAsEnum(parameters, "MPHcyanoMaxValue", context)
    MPHchlThreshForFloatFlag = instance.parameterAsEnum(
        parameters, "MPHchlThreshForFloatFlag", context
    )
    ...
    def create_parameterfile(
        tempdir, MPHvalidPixelExpression, MPHcyanoMaxValueS, MPHchlThreshForFloatFlagS
    ):
        with open(tempdir + "WaterQualityParametersOLCI03.txt", "w") as text_file:
            ...
            text_file.write(
                "mphChlThreshForFloatFlag=" + MPHchlThreshForFloatFlagS + "\n"
            )
    ...
    def execution(tempfolder):
        if folder_check(tempfolder):
            ...
            MPHcyanoMaxValueS, MPHchlThreshForFloatFlagS = convert(
                MPHcyanoMaxValue, MPHchlThreshForFloatFlag
            )
            create_parameterfile(
                tempdir,
                MPHvalidPixelExpression,
                MPHcyanoMaxValueS,
                MPHchlThreshForFloatFlagS,
            )

    execution(tempfolder)

    return {
        "Output_folder": glob.glob(
            os.path.join(tempfile.gettempdir(), tempfolder + "*")
        )[0]
    }
