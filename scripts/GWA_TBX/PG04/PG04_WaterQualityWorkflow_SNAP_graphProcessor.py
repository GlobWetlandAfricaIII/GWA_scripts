...
@alg.input(type=alg.FILE, name="Input_files", label="Select files to be processed", advanced=True, optional=True)
@alg.input(type=alg.FILE, name="Input_folder", label="Input folder", behavior=1, optional=True)
@alg.input(type=alg.STRING, name="ext", label="Define filename extension of input products (e.g. SEN3 or dim)", default="SEN3")
@alg.input(type=alg.NUMBER, name="mem", label="Insert the amount of RAM (inGB) available for processing", default=1, minValue=1, maxValue=31, advanced=True)
@alg.input(type=alg.FOLDER_DEST, name='Output_folder', label='Output folder')
def pg04waterqualityworkflowsnapgraphprocessor(instance, parameters, context, feedback, inputs):
    '''
    ...
    def execution(tempfolder, Output_folder, input_folder, ext, snap_path, param0, param1, param2, param3, param4, NN_PATH):
    ...
    def folder_check(tempfolder):
        try:
            ...
            feedback.pushConsoleInfo('ERROR: Parameter folder could not be found. Please execute steps 1-9 first!')
            return True
    ...
    #        text_file.write('    </node>\n')
            ...
            text_file.write('        <alternativeNNPath>' + NN_PATH + '</alternativeNNPath>\n')
    ...
    #        text_file.write('    </node>\n')
            ...
            text_file.write('        </sources>\n')
            text_file.write('        <parameters>\n')
            text_file.write('            <sourceBands>conc_tsm,conc_chl,rtoa_17,rtoa_8,rhow_17,quality_flags,c2rcc_flags,pixel_classif_flags</sourceBands>\n')
            text_file.write('            <copyMetadata>true</copyMetadata>\n')
            text_file.write('        </parameters>\n')
            text_file.write('    </node>\n')
    ...
    def processing(tempdir, Output_folder, input_files_list, snap_path, gpt_script, paramfile):
        ...
        for n in range(0, len(input_files_list)):
            ...
            feedback.setProgressText('-PtargetbasePath="' + Output_folder + filename[:-ext_len] + '" -e')
    ...
    execution(tempfolder, Output_folder, input_folder, ext, snap_path, param0, param1, param2, param3, param4, NN_PATH)
