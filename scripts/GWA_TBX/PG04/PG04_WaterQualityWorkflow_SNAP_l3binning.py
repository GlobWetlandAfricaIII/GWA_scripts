...
@alg(
    name="pg04waterqualityworkflowsnapl3binning",
    label=alg.tr("PG04_WaterQualityWorkflow_SNAP_l3binning"),
    group="bc",
    group_label=alg.tr("BC")
)

@alg.input(type=alg.FOLDER_DEST, name="folder", label="Input folder")
@alg.input(type=alg.ENUM, name="sensor", label="Select sensor (for time string decoding)", default="OLCI", options=["OLCI", "MERIS", "manual"])
@alg.input(type=alg.STRING, name="start_date_string", label="Select start date for binning period (format: YYYY/MM/DD)", default="2017/01/01")
@alg.input(type=alg.STRING, name="end_date_string", label="Select end date for binning period (format: YYYY/MM/DD)", default="2017/12/31")
@alg.input(type=alg.NUMBER, name="pos1", label="Select end date for binning period (format: YYYY/MM/DD)", default=16, minValue=0, maxValue=200)
@alg.input(type=alg.STRING, name="datepref", label="Define character pattern before date in filename (e.g. R___ for S3A_OL_1_ERR____20161212T...)", default="R____")
@alg.input(type=alg.ENUM, name="binning_type", label="Select binning type", default="period", options=["period", "monthly", "date to date monthly"])
@alg.input(type=alg.NUMBER, name="interval", label="Set binning period (in days) - only required when using binning type 'period'", default=10, minValue=0, maxValue=366)
@alg.input(type=alg.NUMBER, name="res", label="Set spatial resolution (km/px)", default=0.6, minValue=0, maxValue=1000)
@alg.input(type=alg.NUMBER, name="outpref", label="Insert the amount of RAM (inGB) available for processing", default=1, minValue=1, maxValue=31)
@alg.input(type=alg.STRING, name="MPHexportMph", label="Prefix for output filenames (e.g. S3A_OL_ or MER_FRS_)", default="S3A_OL_")
@alg.input(type=alg.FOLDER_DEST, name="outputfolder", label="Output folder")
