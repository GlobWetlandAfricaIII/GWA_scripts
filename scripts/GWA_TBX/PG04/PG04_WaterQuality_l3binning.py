...
@alg.input(
    type=alg.NUMBER,
    name="year",
    label="Select year for processing",
    default=2008,
    minValue=2002,
    maxValue=2012,
)
@alg.input(
    type=alg.NUMBER,
    name="end_month",
    label="Select end month for processing",
    default=1,
    minValue=1,
    maxValue=12,
)
@alg.input(
    type=alg.NUMBER,
    name="res",
    label="Set spatial resolution (km/px)",
    default=0.6,
    minValue=0,
    maxValue=1000,
)
...
def pg04waterqualityworkflowl3binning(instance, parameters, context, feedback, inputs):
    ...
    def create_graph(tempdir, start_month, end_month, year, rows):
        with open(tempdir + "BinningGraph.xml", "w") as text_file:
            ...
            for month in range(start_month, end_month + 1):
                ...
                text_file.write(
                    "      <periodDuration>" + str(days) + ".0</periodDuration>\n"
                )
                text_file.write(
                    "      <timeFilterMethod>TIME_RANGE</timeFilterMethod>\n"
                )
    ...
    def processing(beam_path, gpt_script):
        ...
        cmnd = (
            '"' + beam_path + '/bin/gpt.bat" -c ' + str(mem) + 'G "' + gpt_script + '"'
        )
        feedback.setProgressText(
            '"' + beam_path + '/bin/gpt.bat" -c ' + str(mem) + 'G "'
        )
