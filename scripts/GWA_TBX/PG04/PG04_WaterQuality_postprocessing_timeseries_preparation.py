...
@alg(
    name="pg04waterqualitypostprocessingfortimeseriesolci",
    label=alg.tr("PG04_WaterQuality_PostProcessing_for_timeseries_OLCI"),
    group="bc",
    group_label=alg.tr("BC")
)
@alg.input(type=alg.FILE, name="Input_files", label="Select files to be processed")
@alg.input(type=alg.STRING, name="outpref", label="Output prefix", default="S3A_OL")
...
def algorithm(instance, parameters, context, feedback, inputs):
    """
    PG04_WaterQuality_PostProcessing_for_timeseries_OLCI
    """
    main()


def main(Input_files, outpref, Output_folder):
    import gdal, osr
    import time
    import tempfile
