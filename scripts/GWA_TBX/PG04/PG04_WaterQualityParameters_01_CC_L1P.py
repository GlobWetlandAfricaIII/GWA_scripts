...
@alg.input(
    type=alg.NUMBER,
    name="GlintCloudThresholdAddition",
    label="Glint cloud screening addition",
    default=0.1,
    minValue=0,
)
@alg.input(
    type=alg.BOOL,
    name="OutputCloudProbabilityFeatureValue",
    label="Write cloud probability feature value",
    default=False,
)
@alg.output(type=alg.FILE, name="input_folder", label="Input folder", behavior=1)
def pg04waterqualityparameterscoastcolourl1p(
    ...
    CloudBufferWidth = instance.parameterAsDouble(
        parameters, "CloudBufferWidth", context
    )
    CloudScreeningAmbiguous = instance.parameterAsDouble(
        parameters, "CloudScreeningAmbiguous", context
    )
    CloudScreeningSure = instance.parameterAsDouble(
        parameters, "CloudScreeningSure", context
    )
    ...
    def create_parameterfile(
        ...
        CloudScreeningSure,
        GlintCloudThresholdAddition,
        OutputCloudProbabilityFeatureValue,
    ):
        with open(tempdir + "WaterQualityParameters01.txt", "w") as text_file:
            ...
            text_file.write(
                "cloudScreeningAmbiguous=" + str(CloudScreeningAmbiguous) + "\n"
            )
            text_file.write("cloudScreeningSure=" + str(CloudScreeningSure) + "\n")
            text_file.write(
                "glintCloudThresholdAddition=" + str(GlintCloudThresholdAddition) + "\n"
            )
            text_file.write(
                "outputCloudProbabilityValue="
                + str(OutputCloudProbabilityFeatureValue).lower()
                + "\n"
            )
